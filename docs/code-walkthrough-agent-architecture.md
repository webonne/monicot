...
报告是给人看的，Ledger 是给机器看的，两者用词分开。**报告里不出现 claim / accepted / needs_human / severity / lens 这类内部词**，全部翻译成中文：严重程度写「必须改 / 建议改 / 可以放着」，状态写「必须改的问题 / 我拿不准，你来定 / 查过了，没问题」。

写作规则（写进裁判的 prompt，也约束上游）：

- 小标题就是一句话说清后果：「订单超时重试会重复扣库存」，不是「幂等性缺陷」。
- 失败场景写成人话的因果链：「用户点了提交但网络超时，客户端自动重发，库存被扣两次」，不是「并发场景下存在竞态条件」。这一条由攻击方在写 `failure_scenario` 时就要做到，因为它原样进报告。
- 「怎么改」是一句具体的祈使句，指明改哪里；不确定就写「需要作者决定：A 还是 B」。
- 不写「可能」「建议关注」「值得注意」。
...
**结论：3 个问题必须改完再合，其中 1 个会导致重复扣款；另有 2 处我拿不准，需要你确认。**
...
**会出什么事** 用户点提交，服务端已经扣了库存但响应超时，客户端自动重发。防重复的钥匙是在入口检查的，从扣库存到返回成功之间这段没盖住，库存被扣两次。
**我们验证过** 写了个测试用两个线程模拟超时重发，库存确实被扣两次。复现见 `repro/C-003_test_retry_race.py`。
**作者的说法** 认为钥匙写在事务里应该够用，但同意这段需要实测。
**怎么改** 把防重复检查挪进扣库存的同一个事务，或给扣库存加上按订单号去重。

## 我拿不准，你来定
（每条：在哪 / 担心的是 / 作者的说法 / 为什么没定论 / 你需要判断什么）

## 查过了，没问题
（被有证据驳回的点，一句话说明为什么不成立。这一节本身有价值：它说明哪些地方检查过了）

## 没看到的地方
（简报里标了热点但最后没人深入看的部分）

## 这次走读怎么跑的
（看了几个文件、从哪几个角度、提了多少疑点、多少条实测过、几轮、多少次工具调用；没有测试环境时明确写「所有结论都只是读代码得出的」）
```

空的小节直接省略。「查过了，没问题」超过 8 条时只列 5 条代表，末尾写「另有 N 处同类」。
